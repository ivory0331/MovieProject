...
--USER이(가) "ILIKEMOVIE"입니다.
    ...
    , director             varchar2(100) not null -- 감독 이름
   ...
   -- , star_grade        varchar2(10) default '☆☆☆☆☆' -- 영화 별점 //사실 userRating을 활용할지 말지 고민중임
    ...
    , comment         varchar2(500) not null -- 감상평
    , writedate           date default sysdate    -- 작성일
    , constraint FK_mvComment_tbl_fk_userid foreign key (fk_userid) references mv_member (userid)
    , constraint FK_mvComment_tbl_fk_mvSeq foreign key (fk_mvSeq) references mvdetail_tbl (mvSeq)
    , constraint PK_mvComment_tbl_seq PRIMARY KEY (mvComSeq)
    );
    ...
    ,constraint FK_spoiler_comment_userid foreign key(fk_userid)
                                        references mv_member(userid)
    ,constraint FK_spoiler_comment_Seq foreign key(parentSeq)
                                          references spoiler_tbl(spoilerSeq) on delete cascade
    ...
    ( myPSeq        number                 not null    -- 마이페이지 시퀀스
    , fk_userid      varchar2(50)            not null   -- 사용자ID
    , level             number default '1'                 -- 작가 레벨
    , constraint  FK_myPage_tbl_userid foreign key(fk_userid) references mv_member(userid)
    , constraint PK_myPage_tbl_seq PRIMARY KEY(myPSeq)
    );
    
    -- 마이페이지 게시판 시퀀스
    create sequence myPSeq
    start with 1
    increment by 1
    nomaxvalue
    nominvalue
